...
| fake-indexeddb (npm `fake-indexeddb` 6.2.5, Apache-2.0 per its `package.json` `license` field and `LICENSE` file; dev-only: an in-memory IndexedDB implementation used to test `packages/store`'s IndexedDB backend under vitest/Node, which has no real IndexedDB) | Apache-2.0 | none beyond notice; never shipped to users | **adopted 2026-09-17** at the workspace root (devDependency), used from `packages/store/src/indexeddb.test.ts` |
| jsdom (npm `jsdom` 30.1.0, MIT) with @testing-library/react 16 and @testing-library/dom 10 (MIT); dev-only: a DOM for React hook/component tests under vitest (`// @vitest-environment jsdom` per test file) | MIT | none beyond notice; never shipped to users | **adopted 2026-09-18** at the workspace root (devDependencies), first used by `packages/ui/src/persisted.test.ts` |
