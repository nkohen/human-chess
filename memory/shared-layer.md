...
| `packages/board` | chessground as a React component; only importer of chessground; `MoveLine` (numbered SAN line, hover shows the position; `preview={false}` for exercises) used by every subproject that displays a line (user, 2026-09-16); `BoardEditor` (free-mode position setup with a spare-piece palette, first used by the bot-rating test, 2026-09-16); `Board` `shapes`/`onShapesChange` + `BoardShape`/`BoardBrush` (arrows/circles: editor mode seeds chessground's editable user shapes and reports edits, player mode renders read-only autoShapes mirroring the prop — added 2026-09-18 for the Lesson Builder) | board; later the shared analysis board's board part |
...
| `packages/lessons` (built 2026-09-18) | the lesson content schema for the Lesson Builder: `Lesson`/`LessonStep`/`LessonShape`/`LessonChallenge` (a step = full FEN + prose + arrow/circle shapes + optional move-challenge); `parseLesson`/`parseLessonStep`/`parseLessonList` validators, `serializeLesson`/`parseLessonFile` JSON import/export, `composeFen`/`isLegalFen`/`isLegalMoveFrom`/`sanOfMove` (all through `@human-chess/rules`), `emptyLesson`/`emptyStep` factories. Positions never stored as objects (FEN strings, rebuilt via positionFromFen); challenge answers validated as legal moves via playUci (A1/V3) | lesson authoring; consumed by subprojects/lesson-builder and any future lesson player |
