...
|---|---|
| Openings builder | ~~Priority #2 of the interview: opening-tree analysis over your own played games~~ Built 2026-09-17: "Your games" mode folds your last N lichess or chess.com games (default 100, max 300, 20 plies a side) into a position tree with counts, W/D/L and game links, and adds a browsed line to an opening; own implementation, openingtree's code not reused. Rebuilt 2026-09-17 (openingtree-style analysis): linked accounts now persist and stay in sync (`@human-chess/store`, resumable `syncSourceGames`) instead of a per-load fetch; an expandable move tree (`buildTree`, transposition merging, estimated performance) replaces the flat move list; a filter bar (speed/rated/opponent/date/account); and a diagnostics panel (worst-scoring moves, most-lost positions, per-opening rollup) with "Show" jumping the board/tree to any entry (v) |
